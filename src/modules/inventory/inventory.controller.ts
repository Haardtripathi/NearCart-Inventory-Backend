...
export async function getBalancesController(req: Request, res: Response) {
  ...
  const data = await listBalances(req.auth!.activeOrganizationId!, { ...req.query, branchId } as never);
...
export async function getLedgerController(req: Request, res: Response) {
  const branchId = resolveBranchFilter(req.membership?.branchAccess, (req.query as { branchId?: string }).branchId);
  const data = await listLedger(req.auth!.activeOrganizationId!, { ...req.query, branchId } as never);
