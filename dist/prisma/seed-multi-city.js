...
const GROCERY_PHOTO = "https://images.unsplash.com/photo-1759197894183-ffffa3c7fcd4?w=800&auto=format&fit=crop&q=80";
const PHARMACY_PHOTO = "https://images.unsplash.com/photo-1696861286643-341a8d7a79e9?w=800&auto=format&fit=crop&q=80";
const BAKERY_PHOTO = "https://images.unsplash.com/photo-1774669081553-4ef841520c05?w=800&auto=format&fit=crop&q=80";
const ELECTRONICS_PHOTO = "https://images.unsplash.com/photo-1571857089849-f6390447191a?w=800&auto=format&fit=crop&q=80";
const RESTAURANT_PHOTO = "https://images.unsplash.com/photo-1753727471014-efe38840c7c7?w=800&auto=format&fit=crop&q=80";
const STATIONERY_PHOTO = "https://images.unsplash.com/photo-1761322572550-967ea8c0bfd9?w=800&auto=format&fit=crop&q=80";
const SHOPS = [
    // ---------------------------------------------------------------- Mumbai --
    {
        city: "Mumbai",
...
async function seedShop(shopSeed) {
    console.log(`\n--- ${shopSeed.shopName} (${shopSeed.city}) ---`);
    let organization = await prisma_1.prisma.organization.findUnique({
    ...
    let branch = await prisma_1.prisma.branch.findFirst({
    ...
    for (const productSeed of shopSeed.products) {
        ...
        await prisma_1.prisma.inventoryBalance.create({
            data: {
                organizationId: organization.id,
                branchId: branch.id,
                productId: product.id,
...
async function seedDriver(driverSeed) {
    let driver = await prisma_1.prisma.driver.findUnique({ where: { phone: driverSeed.phone } });
    if (driver) {
    ...
    const passwordHash = await bcrypt_1.default.hash("SeedDriver@123", 12);
    ...
    console.log(`  Created Driver: ${driver.fullName} (${driverSeed.city}) @ (${driverSeed.latitude}, ${driverSeed.longitude}) — VERIFIED, available`);
    return driver;
}
async function main() {
    ...
    const manifestPath = node_path_1.default.join(__dirname, "seed-multi-city.manifest.json");
    (0, node_fs_1.writeFileSync)(manifestPath, JSON.stringify({ generatedAt: new Date().toISOString(), shops: manifestShops, drivers: manifestDrivers }, null, 2));
