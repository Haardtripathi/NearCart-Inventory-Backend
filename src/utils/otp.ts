...
export async function issueOtp(purpose: string, subjectId: string) {
  ...
  const claimedCooldown = await redis.setIfNotExists(
    cooldownKey(purpose, subjectId),
    "1",
...
const OTP_VERIFY_OUTCOME_MESSAGES: Record<string, string> = {
...
export async function verifyOtp(purpose: string, subjectId: string, code: string) {
  ...
  const outcome = await redis.eval(VERIFY_OTP_SCRIPT, [key], [codeHash, env.OTP_MAX_ATTEMPTS]);
  ...
  const message =
    typeof outcome === "string" ? OTP_VERIFY_OUTCOME_MESSAGES[outcome] : undefined;
  throw ApiError.badRequest(message ?? "Incorrect code. Please try again.");
}
