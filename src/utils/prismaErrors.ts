...
export function isUniqueConstraintError(error: Prisma.PrismaClientKnownRequestError): boolean {
  return error.code === "P2002" || /UNIQUE constraint failed/i.test(error.message);
}

export function isForeignKeyConstraintError(error: Prisma.PrismaClientKnownRequestError): boolean {
  return error.code === "P2003" || /FOREIGN KEY constraint failed/i.test(error.message);
}

export function isRecordNotFoundError(error: Prisma.PrismaClientKnownRequestError): boolean {
