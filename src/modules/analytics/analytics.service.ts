 ...
 */
async function countLowStockBalances(organizationId: string, branchId?: string): Promise<number> {
  const rows = await prisma.$queryRawUnsafe<Array<{ lowStockCount: number | bigint }>>(
    `SELECT COUNT(*) AS "lowStockCount"
      ...
      WHERE ib."organizationId" = ?
        ...
        AND ib."onHand" <= MAX(COALESCE(v."reorderLevel", 0), COALESCE(v."minStockLevel", 0), 0)`,
