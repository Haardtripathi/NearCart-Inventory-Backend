...
export interface CatalogMetadataTranslation {
...
export interface CatalogMetadataCategory {
  ...
  isDeleted: boolean;
  sortOrder: number;
  translations: CatalogMetadataTranslation[];
}
...
export interface CatalogMetadataUnit {
  id: string;
  code: string;
  name: string;
  symbol: string | null;
...
export interface OrgCatalogMetadata {
  categories: CatalogMetadataCategory[];
  brands: CatalogMetadataBrand[];
  units: CatalogMetadataUnit[];
}

export interface OrgCatalogMetadataIndex {
  metadata: OrgCatalogMetadata;
  categoryById: Map<string, CatalogMetadataCategory>;
  brandById: Map<string, CatalogMetadataBrand>;
  unitById: Map<string, CatalogMetadataUnit>;
}
...
async function loadOrgCatalogMetadata(organizationId: string): Promise<OrgCatalogMetadata> {
...
function isOrgCatalogMetadata(value: unknown): value is OrgCatalogMetadata {
  return (
    typeof value === "object" &&
    value !== null &&
    Array.isArray((value as OrgCatalogMetadata).categories) &&
    Array.isArray((value as OrgCatalogMetadata).brands) &&
    Array.isArray((value as OrgCatalogMetadata).units)
  );
}

function buildIndex(metadata: OrgCatalogMetadata): OrgCatalogMetadataIndex {
  return {
    metadata,
...
export async function getOrgCatalogMetadata(organizationId: string): Promise<OrgCatalogMetadataIndex> {
  const redis = getCacheRedis();
 ...
 */
export async function ensureCatalogMetadataCoverage(
  index: OrgCatalogMetadataIndex,
  references: { categoryIds: Iterable<string>; brandIds: Iterable<string>; unitIds: Iterable<string> },
): Promise<void> {
 ...
 */
export async function invalidateOrgCatalogMetadata(organizationId: string | null | undefined): Promise<void> {
  if (!organizationId) {
    return;
  }
