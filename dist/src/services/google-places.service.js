...
exports.checkPlaceLocationMatch = checkPlaceLocationMatch;
const env_1 = require("../config/env");
function isGooglePlacesConfigured() {
    return Boolean(env_1.env.GOOGLE_MAPS_API_KEY);
}
 ...
 */
async function checkPlaceLocationMatch(input) {
    ...
    try {
        ...
        const candidates = (body.results ?? [])
            ...
            distanceMeters: Math.round(distanceMeters(input.latitude, input.longitude, result.geometry.location.lat, result.geometry.location.lng)),
        }))
            .sort((a, b) => a.distanceMeters - b.distanceMeters)
            .slice(0, 5);
        const bestMatch = candidates.find((candidate) => nameSimilarity(candidate.name, input.name) >= NAME_MATCH_THRESHOLD);
        return {
