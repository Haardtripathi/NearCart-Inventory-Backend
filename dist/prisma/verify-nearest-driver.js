...
const sales_orders_service_1 = require("../src/modules/sales-orders/sales-orders.service");
const numbering_1 = require("../src/utils/numbering");
const MANIFEST_PATH = "prisma/seed-multi-city.manifest.json";
async function main() {
    const manifest = JSON.parse((0, node_fs_1.readFileSync)(MANIFEST_PATH, "utf-8"));
    const bandra = manifest.shops.find((s) => s.shopSlug === "bandra-fresh-mart");
    if (!bandra) {
    ...
    const actor = await prisma_1.prisma.user.findFirst({ select: { id: true } });
    if (!actor) {
    ...
    const order = await prisma_1.prisma.salesOrder.create({
        data: {
            ...
            orderNumber: (0, numbering_1.generateDocumentNumber)("VERIFY"),
    ...
    await (0, sales_orders_service_1.markSalesOrderReady)(bandra.organizationId, order.id, actor.id);
    const updated = await prisma_1.prisma.salesOrder.findUniqueOrThrow({
        where: { id: order.id },
    ...
    if (updated.assignedDriver) {
        console.log(`Assigned driver: ${updated.assignedDriver.fullName} (${updated.assignedDriver.phone}) @ (${updated.assignedDriver.lastKnownLatitude}, ${updated.assignedDriver.lastKnownLongitude})`);
        console.log(updated.assignedDriver.fullName === "Rajesh Kadam"
