...
export async function sweepStaleDriverAssignments(): Promise<{
  ...
  const staleThreshold = new Date(Date.now() - env.DRIVER_ASSIGNMENT_STALE_MINUTES * 60_000);

  let staleOrders: Array<{
    id: string;
    organizationId: string;
    branchId: string;
    orderNumber: string;
    assignedDriverId: string | null;
    declinedByDriverIds: Prisma.JsonValue;
  ...
  try {
    ...
    staleOrders = await prisma.salesOrder.findMany({
      where: {
        status: SalesOrderStatus.READY,
    ...
    console.warn(
      "[driver-assignment-watchdog] Failed to query stale assignments — skipping this tick",
      error,
    );
  ...
  for (const order of staleOrders) {
    ...
    try {
      const declinedByDriverIds = Array.from(
        new Set([...parseDeclinedDriverIds(order.declinedByDriverIds), staleDriverId]),
      );
      ...
      const updated = await prisma.$transaction(async (tx) => {
        ...
        const { count } = await tx.salesOrder.updateMany({
          where: { id: order.id, assignedDriverId: staleDriverId, status: SalesOrderStatus.READY },
          data: {
            assignedDriverId: null,
            assignedById: null,
            assignedAt: null,
        ...
        await createAuditLog(tx, {
          ...
          entityType: "SalesOrder",
          entityId: order.id,
          before: { status: SalesOrderStatus.READY, assignedDriverId: staleDriverId },
          after: { status: result.status, assignedDriverId: null },
      ...
      try {
        ...
        console.warn(
          `[driver-assignment-watchdog] Re-assignment after timeout failed for order ${updated.id}`,
          error,
        );
      }
      ...
      failed += 1;
      console.warn(
        `[driver-assignment-watchdog] Failed to process stale assignment for order ${order.orderNumber}`,
        error,
      );
    }
  }
  ...
  if (reassigned || unassigned || failed) {
    console.log(
      `[driver-assignment-watchdog] reassigned: ${reassigned}, unassigned (no replacement): ${unassigned}, failed: ${failed}.`,
    );
  }
 ...
 * move with it.
 */
export async function sendNearTimeoutReminders(): Promise<{ reminded: number }> {
  const halfwayMinutes = env.DRIVER_ASSIGNMENT_STALE_MINUTES / 2;
  ...
  let candidates: Array<{ id: string; orderNumber: string; assignedDriverId: string | null }>;

  try {
    candidates = await prisma.salesOrder.findMany({
      where: {
        status: SalesOrderStatus.READY,
        assignedDriverId: { not: null },
    ...
    console.warn(
      "[driver-assignment-watchdog] Failed to query near-timeout reminders — skipping this tick",
      error,
    );
    return { reminded: 0 };
  }
  ...
  for (const order of candidates) {
    ...
    try {
      await sendPushToDriver(order.assignedDriverId, {
      ...
      console.warn(
        `[driver-assignment-watchdog] Failed to send near-timeout reminder for order ${order.orderNumber}`,
        error,
      );
    }
  }
