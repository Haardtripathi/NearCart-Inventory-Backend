...
function toVisionModelUrl(imageUrl) {
    const marker = "/upload/";
...
async function runVisionPrompt(imageUrl, prompt) {
    ...
    try {
        ...
        console.error(isTimeout ? `[replicate] Vision prediction timed out after ${VISION_MODEL_TIMEOUT_MS}ms` : "[replicate] Vision prediction failed:", isTimeout ? undefined : error);
